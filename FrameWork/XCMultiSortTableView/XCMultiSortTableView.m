...
- (void)accessColumnPointCollection {
    ...
    if (columns == 0)
        return;
        //@throw [NSException exceptionWithName:nil reason:@"number of content columns must more than 0" userInfo:nil];
...
- (UITableViewCell *)contentTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(contentDataArray==nil||contentDataArray.count==0||((NSArray*)[contentDataArray objectAtIndex:0]).count==0)
        return cell;
    
    ary = [[contentDataArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
...
- (NSUInteger)rowsInSection:(NSUInteger)section {
    if(leftHeaderDataArray.count==0)
        return 0;
