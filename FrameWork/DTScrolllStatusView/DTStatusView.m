...
- (void)setUpStatusButtonWithTitlt:(NSArray *)titleArray NormalColor:(UIColor *)normalColor SelectedColor:(UIColor *)selectedColor LineColor:(UIColor *)lineColor{
    ...
    for (int i = 0; i < titleArray.count; i++) {
        ...
        //NSLog(@"buttonwidth:%f---x:%f",button.frame.size.width,button.frame.origin.x);
    }
    self.currentIndex = 0;
   
//    NSLog(@"arraycounts:%d",(int)titleArray.count);
    //NSLog(@"widthssss:%f",width);
    //线条
    if (lineColor) {
...
-(void)changeTag:(int)tag
{
    ...
    if (self.lineView) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            //NSLog(@"countsss:%lu",(unsigned long)self.buttonArray.count);
            //NSLog(@"tag:%d",tag);
            //NSLog(@"xss:%f",origin);
